 ...
 {	
	...
	class Actuator
	{
	  public:
		...
		inline ActuatorState  current_state() const { return static_cast<ActuatorState>(digitalRead(pin_number_)); }
		inline void set_current_state(ActuatorState state) { digitalWrite(pin_number_,static_cast<int>(state)); }
	...
	public:
		...
		inline int current_x_value() const { return analogRead(x_pin_number_); }
		inline int current_y_value() const { return analogRead(y_pin_number_); }
		inline int current_button_value() const { return digitalRead(pin_number_); }
